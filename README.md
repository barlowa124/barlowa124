...
| [protein-diffusion](https://github.com/barlowa124/protein-diffusion) | Conditional DDPM + classifier-free guidance over the GB1 fitness landscape, scored against the measured oracle. | The full arc: v1 unconditional model memorized (46% copied rows, novel variants at random fitness); root-caused to a non-smooth landscape; v2 adds fitness conditioning and steers measurably (80% fit vs 4% random, dose-response committed). |
| [dti-fusion](https://github.com/barlowa124/dti-fusion) | Multimodal drug-target interaction on DAVIS: drug fingerprints + ESM-2 protein embeddings with modality ablations, evaluated on held-out proteins. | The ablation table — fusion wins on ranking/MSE, drug-only edges it on MAE, and both are printed. |
| [llm-posttraining](https://github.com/barlowa124/llm-posttraining) | SFT + hand-rolled DPO + GRPO on SmolLM2-135M: abstention-vs-fabrication task, four-stage measured eval on held-out entities. | Full arc measured: DPO collapses deployed behavior at 100% train accuracy, binary rewards give RL zero gradient, shaped GRPO repairs the collapse — raw generations committed. |
